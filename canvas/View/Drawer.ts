...
export default class Drawer {
    ...
    private actionPanel: IActionPanel | null = null;
    private postits: IPostits | null = new Postits();
    private currentPostit: IPostit | null | undefined;
    private postitOffsetX: number = 0;
    private postitOffsetY: number = 0;
    ...
    updateView() {
        if (this.postits) this.actionPanel?.setPostits(this.postits);
        this.draw();
    }
    ...
    mouseDownEvent(evt: MouseEvent | TouchEvent, mx: number, my: number) {
        let index = this.postits?.isInside(mx, my) ?? -1;

        if (index > -1) {
            this.currentPostit = this.postits?.get(index);
            this.postitOffsetX = mx - (this.currentPostit?.getX() ?? 0);
            this.postitOffsetY = my - (this.currentPostit?.getY() ?? 0);
